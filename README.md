...
section is now empty (`_None yet._`) — since filled by two new entries (per-loop today-spend and abort --role, below). Coverage tick 95 (`4778102`) then closed a duplicate-wake
...
surprise. Since then: organize tick `baeca7e` extracted leftover-commit recovery from loop.ts into src/leftover.ts — the self-contained ahead-check → shared review gate → merge-or-warn flow now lives in its own module (`recoverLeftover`, borrowing only identity, the loop's shared gate wiring, usage folding, and the shared merge landing through a `LeftoverContext`), so loop.ts keeps just the tick lifecycle around it; dry tick `edd332e` extracted the shared `promptPreview()` for queued-prompt previews — the 80-char cap in inbox.ts is now the single width behind the prompt_enqueued/prompt_cancelled event previews, the dashboards' inboxPrompts (status.ts), and the CLI's cancel output (cli.ts), surrogate-safe via truncate; coverage tick `ec1850e` then added test/leftover.test.ts — eight units over recoverLeftover's gate-decision branches (no leftover; approved merge with usage folded in; exempt diff without a reviewer run; unmergeable warned and left to the reset; shutdown mid-review failing closed; rejected already reset by the gate; failed under the strike cap kept for re-review; failed at the cap discarded). Since then: plan tick `22d9ed9` added a Planned entry for per-loop today spend — a `today` column on both dashboards beside each loop's lifetime cost (the daily-budget badge shows fleet-wide today-spend and the per-tick usage lines show individual ticks, but nothing answers "which loop spent the day?" at a glance) — and re-audited the done steward entry against main: items (a), (b1), and (b3) verified landed by feature tick 68 (`bf42c98`), remainder dogfood observation only. Clean tick `9d5623d` added the missing doc comments to inbox.ts's enqueuePrompt and inboxSize, and plan tick `503f5ac` then added a second Planned entry — `tumwater abort --role <id>`, killing one loop's in-flight tick via a per-role marker file (work discarded by worktree reset, no director requeue, the loop stays enabled; deliberately distinct from shutdown's `"aborted"` result). Feature tick 83 (`c17893d`) then landed the today-spend code: status-render.ts's table carries `today` between `cost` and `last tick` (per-loop `dailyCost`, `$0.00` on a stale or missing day-stamp; the totals row sums `fleetDailyCost`, by construction equal to the header badge's spend while enabled), gui.ts's payload carries `todayUsd` per loop, and gui-page.ts renders it client-side like cost — but its src-only landing skipped the plan's test updates: two pre-existing layout assertions (gui.test.ts's `<th>cost</th><th>last tick</th>` header match; status-render.test.ts's narrow-width fixture, which addresses columns positionally) now fail against the shifted indices, leaving main's suite red at 551/553 — recorded in BUGS.md's Open section. Coverage tick `86e6559` then fixed a transcript-tail bug it found while testing blank-line handling: readTranscriptTail's backward scan wrapped to the log's last newline when its scanned region began with a zero-length line (a pi log whose first byte is `\n`), re-emitting every line in its window as duplicates until the candidate count reached limit — one guard advances past that leading blank before walking, and a regression test pins tail == full re-read at limits 1/2/50 (recorded under BUGS.md's Fixed section). Plan tick 90 (`56088cf`) then re-audited the today-spend plan against main: its Approach is stale — everything it describes already exists from feature tick 83, and no part of it should be re-implemented — so the remainder was re-specified as three independently pickable pure-test items: (a) repair the two broken layout assertions above (the open bug; landing it makes main green again and moves that BUGS.md entry to Fixed), (b) the TUI/one-shot AC tests in test/status-render.test.ts, and (c) the GUI AC tests in test/gui.test.ts. Since then: coverage tick `67fb3d9` added direct unit tests for src/cli-args.ts — fifteen units over every parser (fail, the count/port/role flag parsers, unknown-flag rejection, and the init/prompt arg parsers); dry tick `e6a0dab` extracted a `parsePositiveInt` helper in cli-args.ts as the one definition of a valid positive integer shared by CLI flag parsing and the GUI's query-param validation (each caller keeps its own missing-value check and failure mode; bounded variants like --port add their upper limit); perf tick `66311f4` then cached queued-prompt reads per poll — `queuedPrompts` serves an unchanged file from the stat-keyed cache (one stat per file per poll instead of re-reading each prompt in full every second until the director consumes it) and skips a file that vanishes mid-listing, while snapshot() derives the inbox count from that same single listing so the header badge can never disagree with its numbered previews. Since then: feature tick `02a8661` closed all three remainders of the today-spend plan — item (a) repaired both broken layout assertions (gui.test.ts's header regex now pins `<th>cost</th><th>today</th><th>last tick</th><th>last result</th>`; status-render.test.ts renumbered its positional indices for the index-7 `today` column), and items (b)/(c) landed the plan's TUI/one-shot and GUI acceptance tests — moving it to Done in PLANS.md and closing the red-suite bug, leaving `tumwater abort --role <id>` as PLANS.md's sole Planned entry; clean tick `887b0c8` dropped test/leftover.test.ts's unused tmpdir import; coverage tick `e81693a` added prompt --cancel's concurrent-dequeue gone path — a queued prompt the director dequeues between listing and cancel reports "no longer queued" (exit 0, not an error) — in test/cli.test.ts with inbox-level units. Since then: coverage tick `f7d3463` added a GUI test pinning that a handler error returns JSON 500 while the server keeps serving (test/gui.test.ts); plan tick `1a704bc` re-audited PLANS.md's sole Planned entry — `tumwater abort --role <id>` — against main and closed three spec gaps: an aborted director author-run must explicitly clear its dequeued `pendingUserPrompt`, a marker for a valid-but-disabled role (no runner) is consumed like an idle loop with no event, and the CLI confirmation gains one clause when `--role` is the director that its in-flight prompt will be discarded. Current main (`f7d3463`): build clean, suite 580/580, verified 2026-09-03.
<!-- tumwater:status:end -->
