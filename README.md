...
build break is fixed (BUGS.md). The plan loop audited it on 2026-08-28 (`ceb6019`, suite green) and re-audited it on 2026-08-30: what remains is test gaps — no role-prompt contract tests (a second 2026-08-30 re-audit pinned them as a sibling block in test/prompt.test.ts, where organize tick `c3779b8` moved the qa role's contract tests when it merged qa-role.test.ts into the module-named files), plus two of the three pickable interval items the re-audit specified after finding tick()'s scheduling branches already unit-covered in test/state.test.ts — coverage tick `be6dc56` landed the isEligible min-gap honoring a per-role override at scheduler level (test/orchestrator.test.ts), leaving an e2e that a per-role override reaches the persisted nextRunAt through a real tick, and validation rejecting a negative per-role interval — and dogfood pending (no `tumwater(steward)` commit in history yet). Self-explaining commit bodies has landed in code (feature
...
against the diff; the plan loop audited it on 2026-08-28 (verified at b101c02, suite green). Its first test gap has since closed — coverage tick `d930959` landed the tick-level e2e in test/loop.test.ts: a compliant fake-pi reply produces a real commit on main carrying subject + WHY/RISK/VERIFIED body (contract order) + trailer, read from actual `git log`, and a SUMMARY-only reply commits subject + trailer only with no body paragraph — leaving one gap against its acceptance criteria — the plan loop re-audited it on 2026-08-31 (`09a7556`) and re-specified the turn-counter coverage as three pickable test items, all pure test work with no src change: parser-level `parser.turns` over message_end events in test/pi.test.ts; a changed-tick e2e whose trailer sums both runs' turns across a transient retry (test/loop.test.ts); and conflict-resolution runs staying out of the count. The review gate has landed end-to-end (feature tick 47): every
