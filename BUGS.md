...
**Files:** src/review.ts (rejection headline); src/build-check.ts (`clipBuildTail` + `ERROR_MESSAGE_LINE`); src/main-baseline.ts (`failureHeadline` doc); test/review.test.ts ("gate pre-check names the failing assertion, not the stack frame the tail opens on"); test/build-check.test.ts (two `clipBuildTail` cases); test/main-baseline.test.ts ("failureHeadline names an unhandled error whose message the tail window would otherwise cut").
