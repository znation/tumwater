...
### Leftover recovery re-reviews a high-friction commit without its high-friction flag, so the reviewer skips the extra scrutiny the flag exists to trigger (found by telemetry loop 2026-09-19)

**Symptom:** The failure digest's top warning cluster is `high-friction tick: 41 turns in 5 min (thresholds: 40 turns / 60 min)`, 3× across `clean`, `coverage`, and `improve` (2026-09-19). The alarm itself is correct, but its follow-through is lost on the one landing path that re-reviews an already-committed change: leftover recovery. `recoverLeftover` calls `this.land({ role, sha, tick, summary, sessionSuffix: "-recovery" })` (src/loop.ts:580) and never sets `highFriction`, so `reviewPinnedChange` (src/lander.ts:89) passes `undefined` into `reviewAheadOfMain` → `buildReviewPrompt`, which then omits the "This change was flagged HIGH-FRICTION … apply extra scrutiny to whether the change should exist at all" paragraph (src/prompt.ts:353). The flag is already durable on the commit — `commitTrailer` stamps a `Friction: high (<turns> turns / <minutes>m)` line (src/commit-message.ts:86) — but nothing reads it back. Correlated commit: **831a8bc** ("Test pi-models' exists-but-unreadable definitions-file branch") carries `Friction: high (44 turns / 4m)` and was landed 2026-09-19 13:43 as "recovered leftover work from coverage", exactly the recovered-high-friction case the digest shows; d3485fe (dry) and 159fb71 (readme) recovered in the same window. A recovered commit also loses its `body` (the same `LandRequest` is built without it), so the reviewer sees neither the friction flag nor the author's WHY/RISK/VERIFIED.

**Repro:** Static: `grep -n highFriction src/loop.ts` shows it set only on the fresh-tick path (line 753) and absent from the recovery `land` call (line 580); `reviewPinnedChange` forwards `req.highFriction` verbatim. End-to-end: make a tick exceed `thrashTurns` so its commit carries a `Friction: high (…)` trailer, kill the harness after the commit is pinned but before the landing drains, then restart — the recovery landing's reviewer prompt lacks the HIGH-FRICTION paragraph. `git show 831a8bc` reproduces the trailer half deterministically.
...
**Suspected cause:** `LandRequest.highFriction` is threaded only from the fresh tick's in-memory boolean; `recoverLeftover`'s `LeftoverContext.land` signature has no field for it and the commit trailer is never parsed, so recovery silently downgrades the gate. The trailer is harness-stamped truth already stored in git, so recovery can reconstruct both the flag and the body from the pinned commit's message without a model run.

**Files:** src/loop.ts (`recoverLeftover` call ~580, `land` ~279), src/leftover.ts (`LeftoverContext.land`), src/lander.ts (`reviewPinnedChange`, `landChange`), src/commit-message.ts (`commitTrailer`), src/prompt.ts (`buildReviewPrompt` high-friction branch).
