...
### Build-check rejections name the stack frame instead of the failing test, so the author's next tick cannot see what broke (found by telemetry loop 2026-09-19)

**Symptom:** Every review rejection in the 2026-09-19 window whose text is machine-generated opens on a stack frame: `build check failed (test): at TestContext.<anonymous> (file:///Users/zach/tumwater/.tumwater/worktrees/_land-clean/dist/…` (2× `clean`), `build check failed (test): at process.processTicksAndRejections (node:internal/process/task_queues:104:5)` (1× `bugfix`), and `build check failed (test): at TestContext.<anonymous> (file:///…/_land-coverage/dist/…)` (1× `coverage`) — four rejections across three roles, all on 09-19. `clipBuildTail` (`src/build-check.ts`) keeps the LAST ten lines of the check's output, so a suite that dies on an unhandled rejection opens mid-stack; `src/review.ts:171` then takes `(outcome.outputTail ?? [])[0]` as the headline and `src/review.ts:174` joins it into the reason. That string resets the branch and is injected into the author's next tick, so the author is told where the suite died, never what assertion failed. The 09-18 fix `bb8dc26` ("Stop a one-worktree red main from latching, and name what actually broke") built exactly the right helper — `failureHeadline` (`src/main-baseline.ts:22`, first line that is not an `/^at\s/` frame, pinned by `test/main-baseline.test.ts:59`) — but wired it only into `mainRedGate`; the review gate's rejection path was left on the raw first line. The week's one red-main warning shows the helper's remaining gap on the same tail shape: `main 67a1b336 is red (test: errno: -2,)` — `failureHeadline` skips only `at ` frames, so an unhandled `Error`'s object-property line (`errno: -2,`) still wins over the `Error: ENOENT …` message that would name the failure.

**Repro:** Static: `grep -n outputTail src/review.ts` — the rejection headline (line 171) is `outputTail[0]`; `grep -n failureHeadline src/review.ts` finds no caller, while `test/main-baseline.test.ts:59` already pins `failureHeadline(["at process.processTicksAndRejections (…)", "at async Promise.all (index 0)", "AssertionError [ERR_ASSERTION]: …"])` to the assertion line. End-to-end: a build-check tail whose last ten lines begin `at TestContext.<anonymous> (…)` yields the rejection reason `build check failed (test): at TestContext.<anonymous> (…)`, i.e. a frame, where `failureHeadline` on the same array returns the assertion/diff line. The red-main sibling is reproducible by tailing a Node ENOENT dump far enough that the `Error: ENOENT …` line is clipped and only its property lines survive: `failureHeadline` returns `errno: -2,`.

**Expected:** a machine-generated rejection reason injected into the author's next tick must name the failure the way the red-main warning now does — reuse `failureHeadline` for the headline (the rest of the clipped tail still follows, so the fix costs nothing). Extend `failureHeadline` to also skip error-object property lines (`^\s*\w+:\s.*,$`) or to prefer a line bearing the error message, so the red-main warning names the failing test instead of `errno: -2,`. One shared headline helper, both callers.

**Suspected cause:** `failureHeadline` was added only to the red-main path; `review.ts` still predates it and uses the first tail line unconditionally. The helper's frame test is also narrower than the tail shapes `clipBuildTail` can hand it (a `node:test` failure tail is frames plus an error-object dump plus the assertion line).

**Files:** src/review.ts (rejection headline, ~line 171/174), src/main-baseline.ts (`failureHeadline`), test/review.test.ts (a build-check-failure rejection pins its reason to the assertion line), test/main-baseline.test.ts (the error-property-line case).
