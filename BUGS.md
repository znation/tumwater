...
### The failure digest's per-role rejection counter reads 0 on the live fleet: it counts a `tick_end` result no landing path emits (found by telemetry loop 2026-09-19)
...
**Files:** src/failure-report.ts (`roleStats` rejection source, lines 186–198; the delta render at ~389), test/failure-report.test.ts (`deltas count quiet kills and rejections per role, both windows`, and a realistic-fixture case pairing a `queued` tick_end with a `review_rejected` event).
